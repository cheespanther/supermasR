analisisticketsmensual <- function(year, tienda, mes, summarize = FALSE) 
{
  ##library(lubridate)
  library (plyr)
  ...
  nombres <<- c("Fecha", "Sesion", "Cajero", "Total", "Tienda")
  colnames(combined) <<- nombres
  datatickets <<- c(combined$Total, na.rm= TRUE)
  numerodatos <- nrow(combined)
  soloventas <<- subset (datatickets, datatickets > 0)
  ventastotales <<- sum(datatickets)
  combinedmean <<- mean(datatickets)
  ...
  print(tablabonita)
  message("Promedio por Cajero:")
  print(promedio_cajeros)
  
  setwd("D:/KEEP_ISKAR/R/datos/TICKETS/reportes")
  write.csv(tablabonita, file = paste(tienda, year, mes, ".csv", sep=""))
  write.csv(resumentabla, file = paste(tienda, year, mes, "resumen.csv", sep=""))
  write.csv(promedio_cajeros, file = paste(tienda, year, mes, "cajeros.csv", sep=""))
